    ...
    async def _gen_evaluate_privacy(
        ...
        for rule in prepended_rules:
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Prepended privacy rule %s of EntTestObject3 with ID %s was denied for %s",
                    type(rule),
                    self.id,
                    str(vc),
                )
        ...
        if isinstance(config, EdgeDelegate):
            ...
            if decision != Decision.PASS:
                return decision
            privacy_logger.debug(
                "Defaulting to denying access to EntTestObject3 with ID %s after privacy rule returned PASS for %s",
                self.id,
                str(vc),
            )
