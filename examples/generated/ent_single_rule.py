...
class EntSingleRule(Ent[ExampleViewerContext]):
    vc: ExampleViewerContext
    model: EntSingleRuleModel
    m = EntSingleRuleModel

    def __init__(self, vc: ExampleViewerContext, model: EntSingleRuleModel) -> None:
        self.vc = vc
        self.model = model
    ...
    async def _gen_evaluate_privacy(
        ...
        for rule in prepended_rules:
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Prepended privacy rule %s of EntSingleRule with ID %s was denied for %s",
                    type(rule),
                    self.id,
                    str(vc),
                )
        ...
        if isinstance(config, EdgeDelegate):
            ...
            if decision != Decision.PASS:
                return decision
            privacy_logger.debug(
                "Defaulting to denying access to EntSingleRule with ID %s after privacy rule returned PASS for %s",
                self.id,
                str(vc),
            )
            ...
            return Decision.DENY
        raise ExecutionError(
            "An invalid privacy configuration was found for EntSingleRule: invalid config type"
        )
    ...
    async def _gen_no_privacy_DO_NOT_USE(
        ...
        model = await session.get(
            EntSingleRuleModel, real_ent_id, with_for_update=for_update or None
        )
        if model is None:
            return None
        session.info.setdefault("cache", set()).add(model)
        return EntSingleRule(vc=vc, model=model)

    @classmethod
    async def _genx_no_privacy_DO_NOT_USE(
        cls, vc: ExampleViewerContext, ent_id: UUID | str, for_update: bool = False
    ) -> EntSingleRule:
        ent = await EntSingleRule._gen_no_privacy_DO_NOT_USE(vc, ent_id, for_update)
        if ent is None:
            raise EntNotFoundError(f"No EntSingleRule found for ID {ent_id}")
        return ent
    ...
    async def gen(
        ...
        async with emulate_for_update(
            session, EntSingleRuleModel, "id", real_ent_id, for_update
        ):
            model = await session.get(
                EntSingleRuleModel, real_ent_id, with_for_update=for_update or None
            )
    ...
    async def _gen_from_model(
        cls, vc: ExampleViewerContext, model: EntSingleRuleModel | None
    ) -> EntSingleRule | None:
        if not model:
            return None
        ent = EntSingleRule(vc=vc, model=model)
    ...
    async def _genx_from_model(
        cls, vc: ExampleViewerContext, model: EntSingleRuleModel
    ) -> EntSingleRule:
        ent = await EntSingleRule._gen_from_model(vc=vc, model=model)
        if not ent:
            raise EntNotFoundError(f"No EntSingleRule found for ID {model.id}")
        return ent
