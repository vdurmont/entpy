...
class EntDelegatingChild(Ent[ExampleViewerContext]):
    ...
    async def _gen_evaluate_privacy(
        ...
        for rule in prepended_rules:
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Prepended privacy rule %s of EntDelegatingChild with ID %s was denied for %s",
                    type(rule),
                    self.id,
                    str(vc),
                )
        ...
        if isinstance(config, EdgeDelegate):
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Delegate privacy of EntDelegatingChild with ID %s to edge %s was denied for %s",
                    self.id,
                    config.edge_name,
                    str(vc),
                )
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Privacy rule %s of EntDelegatingChild with ID %s was denied for %s",
                    type(config),
                    self.id,
                    str(vc),
                )
            ...
            if decision != Decision.PASS:
                return decision
            privacy_logger.debug(
                "Defaulting to denying access to EntDelegatingChild with ID %s after privacy rule returned PASS for %s",
                self.id,
                str(vc),
            )
            ...
            for item in config:
                if isinstance(item, PrivacyRule):
                    ...
                    raise ExecutionError(
                        "An invalid privacy configuration was found for EntDelegatingChild: invalid item type in list"
                    )
                # If we get an ALLOW or DENY, we return instantly. Else, we keep going.
                if decision != Decision.PASS:
                    return decision
            ...
            privacy_logger.debug(
                "Defaulting to denying access to EntDelegatingChild with ID %s after exhausting all privacy rules for %s",
                self.id,
                str(vc),
            )
            return Decision.DENY
