...
class EntTestObject(IEntTestThing, Ent[ExampleViewerContext]):
    ...
    async def _gen_evaluate_privacy(
        ...
        for rule in prepended_rules:
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Prepended privacy rule %s of EntTestObject with ID %s was denied for %s",
                    type(rule),
                    self.id,
                    str(vc),
                )
        ...
        if isinstance(config, EdgeDelegate):
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Delegate privacy of EntTestObject with ID %s to edge %s was denied for %s",
                    self.id,
                    config.edge_name,
                    str(vc),
                )
            ...
            if decision != Decision.PASS:
                return decision
            privacy_logger.debug(
                "Defaulting to denying access to EntTestObject with ID %s after privacy rule returned PASS for %s",
                self.id,
                str(vc),
            )
            ...
            for item in config:
                if isinstance(item, PrivacyRule):
                    ...
                    raise ExecutionError(
                        "An invalid privacy configuration was found for EntTestObject: invalid item type in list"
                    )
                # If we get an ALLOW or DENY, we return instantly. Else, we keep going.
                if decision != Decision.PASS:
                    return decision
            ...
            privacy_logger.debug(
                "Defaulting to denying access to EntTestObject with ID %s after exhausting all privacy rules for %s",
                self.id,
                str(vc),
            )
            return Decision.DENY
    ...
    async def _genx_no_privacy_DO_NOT_USE(
        ...
        ent = await EntTestObject._gen_no_privacy_DO_NOT_USE(vc, ent_id, for_update)
        if ent is None:
...
class EntTestObjectQuery(EntQuery[EntTestObject, EntTestObjectModel]):
    ...
    def _finalize_query(self) -> Select:
        if self.include_soft_deleted:
            ...
            return self.query.where(EntTestObjectModel.soft_deleted_at.is_(None))
    ...
    async def genx_first(self, for_update: bool = False) -> EntTestObject:
        ...
        if not ent:
            raise EntNotFoundError("Expected to find a EntTestObject, got None.")
        return ent
    ...
    def with_soft_deleted(self) -> "EntTestObjectQuery":
