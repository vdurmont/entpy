    ...
    async def _gen_evaluate_privacy(
        ...
        if isinstance(config, EdgeDelegate):
            privacy_logger.debug(
                "Delegating privacy of EntDelegatingGrandchild with ID %s to edge %s",
                self.id,
                config.edge_name,
            )
            ...
            for rule in config:
                ...
                privacy_logger.debug(
                    "Privacy rule %s of EntDelegatingGrandchild with ID %s returned %s",
                    type(rule),
                    self.id,
                    decision,
                )
            ...
            privacy_logger.debug(
                "Defaulting to denying access to EntDelegatingGrandchild with ID %s after exhausting all privacy rules.",
                self.id,
            )
