...
class EntPassThenDeny(Ent[ExampleViewerContext]):
    ...
    async def _gen_evaluate_privacy(
        ...
        for rule in prepended_rules:
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Prepended privacy rule %s of EntPassThenDeny with ID %s was denied for %s",
                    type(rule),
                    self.id,
                    str(vc),
                )
        ...
        config = EntPassThenDenySchema().get_privacy_config(action)
        if isinstance(config, EdgeDelegate):
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Delegate privacy of EntPassThenDeny with ID %s to edge %s was denied for %s",
                    self.id,
                    config.edge_name,
                    str(vc),
                )
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Privacy rule %s of EntPassThenDeny with ID %s was denied for %s",
                    type(config),
                    self.id,
                    str(vc),
                )
            ...
            if decision != Decision.PASS:
                return decision
            privacy_logger.debug(
                "Defaulting to denying access to EntPassThenDeny with ID %s after privacy rule returned PASS for %s",
                self.id,
                str(vc),
            )
            ...
            for item in config:
                if isinstance(item, PrivacyRule):
                    ...
                    raise ExecutionError(
                        "An invalid privacy configuration was found for EntPassThenDeny: invalid item type in list"
                    )
                # If we get an ALLOW or DENY, we return instantly. Else, we keep going.
                if decision != Decision.PASS:
                    return decision
            ...
            privacy_logger.debug(
                "Defaulting to denying access to EntPassThenDeny with ID %s after exhausting all privacy rules for %s",
                self.id,
                str(vc),
            )
            return Decision.DENY
        raise ExecutionError(
            "An invalid privacy configuration was found for EntPassThenDeny: invalid config type"
        )

    async def _gen_load_delegate(self, vc: ExampleViewerContext, edge_name: str) -> Ent:
        raise ExecutionError(
            f"An invalid privacy configuration was found for EntPassThenDeny: could not find delegate for {edge_name}"
        )

    @classmethod
    async def _gen_no_privacy_DO_NOT_USE(
        cls, vc: ExampleViewerContext, ent_id: UUID | str, for_update: bool = False
    ) -> EntPassThenDeny | None:
        real_ent_id = validate_ent_id(ent_id)
        session = get_session()
        model = await session.get(
            EntPassThenDenyModel, real_ent_id, with_for_update=for_update or None
        )
        if model is None:
            return None
        session.info.setdefault("cache", set()).add(model)
        return EntPassThenDeny(vc=vc, model=model)

    @classmethod
    async def _genx_no_privacy_DO_NOT_USE(
        cls, vc: ExampleViewerContext, ent_id: UUID | str, for_update: bool = False
    ) -> EntPassThenDeny:
        ent = await EntPassThenDeny._gen_no_privacy_DO_NOT_USE(vc, ent_id, for_update)
        if ent is None:
            raise EntNotFoundError(f"No EntPassThenDeny found for ID {ent_id}")
        return ent
    ...
    async def gen(
        ...
        async with emulate_for_update(
            session, EntPassThenDenyModel, "id", real_ent_id, for_update
        ):
            model = await session.get(
                EntPassThenDenyModel, real_ent_id, with_for_update=for_update or None
            )
    ...
    async def _gen_from_model(
        cls, vc: ExampleViewerContext, model: EntPassThenDenyModel | None
    ) -> EntPassThenDeny | None:
        if not model:
            return None
        ent = EntPassThenDeny(vc=vc, model=model)
    ...
    async def _genx_from_model(
        cls, vc: ExampleViewerContext, model: EntPassThenDenyModel
    ) -> EntPassThenDeny:
        ent = await EntPassThenDeny._gen_from_model(vc=vc, model=model)
        if not ent:
            raise EntNotFoundError(f"No EntPassThenDeny found for ID {model.id}")
        return ent
...
class EntPassThenDenyQuery(EntQuery[EntPassThenDeny, EntPassThenDenyModel]):
    vc: ExampleViewerContext
    ...
    def _finalize_query(self) -> Select:
        if self.include_soft_deleted:
            ...
            return self.query.where(EntPassThenDenyModel.soft_deleted_at.is_(None))

    async def _gen_ents(
        self, result: Result[tuple[EntPassThenDenyModel]]
    ) -> list[EntPassThenDeny | None]:
        models = result.scalars().all()
        return [
            await EntPassThenDeny._gen_from_model(self.vc, model)  # noqa: SLF001
            for model in models
        ]

    async def gen_first(self, for_update: bool = False) -> EntPassThenDeny | None:
        session = get_session()
    ...
    async def _gen_ent(
        ...
        return await EntPassThenDeny._gen_from_model(self.vc, model)  # noqa: SLF001

    async def genx_first(self, for_update: bool = False) -> EntPassThenDeny:
        ent = await self.gen_first(for_update)
        if not ent:
            raise EntNotFoundError("Expected to find a EntPassThenDeny, got None.")
        return ent
    ...
    def order_by_id_asc(self) -> "EntPassThenDenyQuery":
        self.query = self.query.order_by(EntPassThenDenyModel.id.asc())
        return self

    def order_by_id_desc(self) -> "EntPassThenDenyQuery":
        self.query = self.query.order_by(EntPassThenDenyModel.id.desc())
        return self

    def with_soft_deleted(self) -> "EntPassThenDenyQuery":
