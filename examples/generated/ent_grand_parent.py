...
class EntGrandParent(Ent[ExampleViewerContext]):
    ...
    async def _gen_evaluate_privacy(
        ...
        if isinstance(config, EdgeDelegate):
            privacy_logger.debug(
                "Delegating privacy of EntGrandParent with ID %s to edge %s",
                self.id,
                config.edge_name,
            )
            ...
            for rule in config:
                ...
                privacy_logger.debug(
                    "Privacy rule %s of EntGrandParent with ID %s returned %s",
                    type(rule),
                    self.id,
                    decision,
                )
    ...
    async def _gen_no_privacy_DO_NOT_USE(
        ...
        model = await session.get(
            EntGrandParentModel, real_ent_id, with_for_update=for_update
        )
        if model is None:
            return None
    ...
    async def _genx_no_privacy_DO_NOT_USE(
        ...
        ent = await EntGrandParent._gen_no_privacy_DO_NOT_USE(vc, ent_id)
        if ent is None:
    ...
    async def gen(
        ...
        async with emulate_for_update(
            session, EntGrandParentModel, "id", real_ent_id, for_update
        ):
            model = await session.get(
                EntGrandParentModel, real_ent_id, with_for_update=for_update
            )
