        ...
        if isinstance(config, EdgeDelegate):
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Delegate privacy of EntTestSubObject with ID %s to edge %s was denied",
                    self.id,
                    config.edge_name,
                )
            return decision
            ...
            for rule in config:
                ...
                if decision == Decision.DENY:
                    privacy_logger.debug(
                        "Privacy rule %s of EntTestSubObject with ID %s was denied",
                        type(rule),
                        self.id,
                    )
            ...
            privacy_logger.debug(
                "Defaulting to denying access to EntTestSubObject with ID %s after exhausting all privacy rules.",
                self.id,
            )
