    ...
    async def _gen_evaluate_privacy(
        ...
        for rule in prepended_rules:
            ...
            if decision == Decision.DENY:
                privacy_logger.debug(
                    "Prepended privacy rule %s of EntMixedList with ID %s was denied for %s",
                    type(rule),
                    self.id,
                    str(vc),
                )
        ...
        if isinstance(config, EdgeDelegate):
            ...
            if decision != Decision.PASS:
                return decision
            privacy_logger.debug(
                "Defaulting to denying access to EntMixedList with ID %s after privacy rule returned PASS for %s",
                self.id,
                str(vc),
            )
            ...
            return Decision.DENY
        raise ExecutionError(
            "An invalid privacy configuration was found for EntMixedList: invalid config type"
        )
