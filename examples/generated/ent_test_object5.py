...
class EntTestObject5(Ent[ExampleViewerContext]):
    ...
    async def _gen_evaluate_privacy(
        ...
        if isinstance(config, EdgeDelegate):
            privacy_logger.debug(
                "Delegating privacy of EntTestObject5 with ID %s to edge %s",
                self.id,
                config.edge_name,
            )
            ...
            for rule in config:
                ...
                privacy_logger.debug(
                    "Privacy rule %s of EntTestObject5 with ID %s returned %s",
                    type(rule),
                    self.id,
                    decision,
                )
            ...
            privacy_logger.debug(
                "Defaulting to denying access to EntTestObject5 with ID %s after exhausting all privacy rules.",
                self.id,
            )
    ...
    async def _gen_no_privacy_DO_NOT_USE(
        ...
        model = await session.get(
            EntTestObject5Model, real_ent_id, with_for_update=for_update
        )
        if model is None:
            return None
    ...
    async def _genx_no_privacy_DO_NOT_USE(
        ...
        ent = await EntTestObject5._gen_no_privacy_DO_NOT_USE(vc, ent_id)
        if ent is None:
